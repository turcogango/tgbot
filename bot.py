...
async def veri(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if panel1_data:
            text += "<b>PANEL 1 (Paypanel)</b>\n\n"
            for k, v in panel1_data.items():
                text += (
                    f"{k}\n"
                    f'<span style="color:green;">Yat:</span> {format_number(v["yat"])} ({v["yat_adet"]} adet)\n'
                    f'<span style="color:red;">Çek:</span> {format_number(v["cek"])} ({v["cek_adet"]} adet)\n\n'
                )
        
        if panel2_data:
            text += "<b>PANEL 2 (TronPanel)</b>\n\n"
